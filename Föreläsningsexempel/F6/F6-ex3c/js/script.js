...
function newUserMarker(e) {
	...
	infoWindow.setContent("<form id='newPlaceForm'><p>Plats: <input id='newPlace' size='10'></p><p><input type='button' value='Spara' onclick='saveUserMarker()'></p></form>");
	infoWindow.open(myMap,userMarker);
} // End newUserMarker

// Spara den nya markören i arrayen med alla markörer
function saveUserMarker() {
	let newTitle = document.getElementById("newPlace").value;
	userMarker.setTitle(newTitle);
	myMarkers.push(userMarker);
	google.maps.event.addListener(userMarker,"click",showInfoWindow);
	infoWindow.close();
} // End saveUserMarker
